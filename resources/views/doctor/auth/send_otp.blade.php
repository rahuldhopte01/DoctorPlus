                ...
                <div class="row g-0">
                    ...
                    <div class="col-md-12 col-lg-7 p-4 p-lg-5">
                        <div class="mb-5 text-center text-lg-start">
                            ...
                            <h3 class="fw-medium text-muted fs-5">{{__('Enter the verification code sent to you.')}}</h3>
                        </div>
                        ...
                        @if (!session('error') && $status)
                            @include('superAdmin.auth.status', ['status' => $status])
                        @endif

                        <form action="{{ url('doctor/verify_otp') }}" method="post" id="otpForm">
                            @csrf
                            ...
                            <div class="mt-4 text-center">
                                <p class="text-muted small">
                                    ...
                                    <a href="{{ url('doctor/send_otp/'.$user->id) }}" class="text-primary text-decoration-none fw-semibold ms-1 transition-all hover-primary">
                                        {{__('Resend Code')}}
                                    </a>
                                </p>
                            </div>
