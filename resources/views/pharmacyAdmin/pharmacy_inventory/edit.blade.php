...
@section('title',__('Edit Pharmacy Inventory'))

@section('content')
<section class="section">
    @include('layout.breadcrumb',[
        'title' => __('Edit Pharmacy Inventory'),
        'url' => url('pharmacy_inventory'),
        'urlTitle' => __('Pharmacy Inventory')
    ])
    <form action="{{ url('pharmacy_inventory/'.$inventory->id) }}" method="post" class="myform">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-body">
                <div class="form-group">
                    ...
                    <select name="medicine_id" class="select2 @error('medicine_id') is-invalid @enderror" required>
                        ...
                        @foreach ($medicines as $medicine)
                            <option value="{{$medicine->id}}" {{ (old('medicine_id', $inventory->medicine_id) == $medicine->id) ? 'selected' : '' }}>
                                {{$medicine->name}} 
                ...
                <div class="form-group">
                    ...
                    <input type="number" min="0" value="{{ old('low_stock_threshold', $inventory->low_stock_threshold) }}" name="low_stock_threshold" class="form-control @error('low_stock_threshold') is-invalid @enderror">
