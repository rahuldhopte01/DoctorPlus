...
<section class="section">
    <div class="d-flex flex-wrap align-items-stretch min-vh-100">
      <div class="col-lg-4 col-md-6 col-12 order-lg-1 min-vh-100 order-2 bg-white d-flex align-items-center justify-content-center">
        <div class="p-4 p-lg-5 w-100" style="max-width: 500px;">
          ...
          <div class="text-center mb-5">
              ...
              <img src="{{ $setting->logo }}" alt="logo" width="180" class="mb-4">
              @else
              <img src="{{url('/images/upload_empty/fuxxlogo.png')}}" alt="logo" width="180" class="mb-4" />
              @endif
              <h4 class="text-dark font-weight-bold mb-2">{{__('Laboratory Portal')}}</h4>
              <p class="text-muted">{{__('Sign in to manage test requests and reports.')}}</p>
          </div>
          ...
          <form method="POST" action="{{ url('verify_pathologist') }}" class="needs-validation" novalidate="">
            ...
            <div class="form-group mb-4">
              ...
              <div class="input-group">
                  ...
                  <input id="email" type="email" class="form-control border-left-0 bg-light @error('email') is-invalid @enderror" name="email" tabindex="1" required autofocus value="{{ old('email') }}" style="border-radius: 0 0.5rem 0.5rem 0;">
              </div>
                @error('email')
            ...
            <div class="form-group mb-4">
              ...
              <div class="input-group">
                  ...
                  <input id="password" type="password" class="form-control border-left-0 bg-light @error('password') is-invalid @enderror" name="password" tabindex="2" required style="border-radius: 0 0.5rem 0.5rem 0;">
              </div>
              @error('password')
                    <div class="invalid-feedback">
                        {{ $message }}
          ...
          <div class="text-center mt-4 pt-3 border-top">
              <span class="text-muted">{{__("Don't have an account?")}}</span> <a href="{{ url('pathologist_sign_up') }}" class="font-weight-bold text-primary">{{ __('Create new one')}}</a>
          </div>
      ...
      <div class="col-lg-8 col-12 order-lg-2 order-1 d-none d-lg-block p-0 position-relative">
          <div class="h-100 w-100" style="background-image: url('{{ url('assets/img/login.png') }}'); background-size: cover; background-position: center;">
              ...
              <div class="position-absolute bottom-0 text-white p-5 w-100">
                  <div class="mb-5 pb-3 px-4">
                      <h1 class="font-weight-bold mb-3" style="font-size: 3rem;">{{ __('Laboratory Partner') }}</h1>
                      <p class="lead" style="max-width: 500px; opacity: 0.9;">Process test requests, upload results, and integrate perfectly with our medical platform.</p>
                  </div>
              </div>
          </div>
      </div>
