...
<div class="xl:w-3/4 mx-auto py-10">
    ...
    <div class="bg-white shadow-xl rounded-lg p-8">
        ...
        <form id="addressForm" method="POST" action="{{ route('questionnaire.save-cannaleo-delivery-address', ['categoryId' => $category->id]) }}">
            @csrf
            ...
            <div class="mb-6">
                ...
                <div class="space-y-3" id="existingAddressList">
                    @foreach($existingAddresses as $address)
                    <label class="flex items-start gap-3 p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-primary transition-all existing-address-label">
                        <input type="radio" name="address_id" value="{{ $address->id }}" class="mt-1 existing-address-radio">
                        <div class="flex-1">
                ...
                <div class="text-center my-4">
                    <span class="text-gray">{{ __('OR enter a new address below') }}</span>
                </div>
            </div>
            @endif
            ...
            <div class="space-y-4">
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block font-fira-sans font-medium text-black mb-2">{{ __('State') }}</label>
...
<script>
// When an existing address is selected, prefill the form fields
document.querySelectorAll('.existing-address-radio').forEach(function(radio) {
    radio.addEventListener('change', function() {
        const label = this.closest('label');
        const addressText = label.querySelector('p.font-fira-sans.text-black').textContent.trim();
