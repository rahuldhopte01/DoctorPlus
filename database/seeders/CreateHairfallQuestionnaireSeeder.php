...
/**
 * Creates a dummy questionnaire for the "hairfall" category
 * Run: php artisan db:seed --class=CreateHairfallQuestionnaireSeeder
 */
class CreateHairfallQuestionnaireSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!$category) {
            $this->command->error('Category "hairfall" not found. Please create it first.');
            return;
        }

        $this->command->info("Found category: {$category->name} (ID: {$category->id})");

        // Check if questionnaire already exists
        $existingQuestionnaire = Questionnaire::where('category_id', $category->id)->first();
        if ($existingQuestionnaire) {
            $this->command->warn("Questionnaire already exists for this category. Deleting old one...");
        ...
        $this->command->info("Created questionnaire: {$questionnaire->name}");
        ...
        QuestionnaireQuestion::create([
            ...
            'conditional_logic' => [
                'show_if' => [
                    'question_id' => null, // Will be set dynamically if needed
                    'operator' => 'equals',
                    'value' => 'Yes'
                ]
            ],
        ...
        $this->command->info("✅ Successfully created questionnaire with 6 sections and " . $questionnaire->questions()->count() . " questions!");
        $this->command->info("Questionnaire ID: {$questionnaire->id}");
        $this->command->info("You can now test the questionnaire at: /questionnaire/category/{$category->id}");
    }
}
